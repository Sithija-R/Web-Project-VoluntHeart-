...
export const editPost = (postData) => async (dispatch) => {
  try {
    const { data } = await api.post(`/api/posts/edit`, postData);
...
export const getSuggestions = (keyword) => async (dispatch) => {
  try {
    const { data } = await axios.get(
      `http://localhost:5555/suggestios/${keyword}`
    );
...
export const getPostById = (postId) => async (dispatch) => {
  try {
    const { data } = await api.get(`/api/posts/get/${postId}`);
...
export const deleteMedia = (uniqueKey,mediaId) => async (dispatch) => {
  try {
    const { data } = await api.delete(`/api/posts/delete/media/${uniqueKey}/${mediaId}`);

    dispatch({ type: MEDIA_DELETE_SUCCESS, payload: data });
  } catch (error) {
    dispatch({ type: MEDIA_DELETE_FAILURE, payload: error.message });
  }
};