...
export const postReducer = (state=initialState,action)=>{
    ...
    switch (action.type) {
        ...
        case DELETE_POST_SUCCESS:
            return{...state,loading:false,error:null,posts:state.posts.filter((post)=>post.id!==action.payload), delete:action.payload};

        case FIND_POST_BY_CONTENT_SUCCESS:
            return{...state,loading:false,error:null,postSearchResult:action.payload };

        case FIND_POST_BY_CONTENT_FAILURE:
            return{...state,loading:false,error:action.payload,postSearchResult:null}

        case COMMENT_POST_SUCCESS:
            return{...state,loading:false,error:null,comment:action.payload };

        case SUGGESTIONS_SUCCESS:
            return{...state,loading:false,error:null,suggestion:action.payload };
