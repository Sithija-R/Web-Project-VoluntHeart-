...
const Postcontainer = ({ item }) => {
  ...
  const handleEdit=()=>{
    dispatch(getPostById(item?.postId))
  }
  ...
  const handleProfileNavigate = () => {
    item.createdBy.userId == auth.user.userId
      ? navigate(`/profile`)
      : navigate(`/profile/${item.createdBy?.userSecret}`);
  };

  const locationurl = `https://maps.google.com/maps?q=${item.location?.lat},${item.location?.lng}&hl=es;&output=embed`;

  return (
    <div className="  w-full px-3 py-3 min-h-40 rounded-3xl shadow-lg border-t-[1px] bg-white cursor-pointer">
...
<Modal
      ...
      >
        <Box sx={editmodelstyle}>
          <div className="flex justify-between text-slate-600 mb-4">
            <h4 className="font-semibold">Edit post</h4>
            <CloseIcon onClick={handleEditModalClose} className="cursor-pointer" />
          </div>
        <EditPost handleModalClose={handleEditModalClose} post={item}/>
        </Box>
      </Modal>
      ...
      <div className="flex items-center justify-between font-semibold text-gray-500 px-2">
        ...
        >
          <Avatar>
            <img src={item.createdBy.profilePic} alt="profile pic" />
          </Avatar>
          <div>
            <p>{item.createdBy.fullName}</p>
            <p className="text-xs text-green-500 ">
              {item.createdBy.donor ? "donor" : "fundraiser"}
            </p>
          </div>
        </div>
        <div className="">
      ...
      <div className="px-14 pt-2 text-sm text-blue-500 cursor-pointer">
        {" "}
        {item.location ? (
        ...
        <div>
          ...
          <form onSubmit={commentformik.handleSubmit}>
            <div className="mt-1 flex justify-between">
              <input
                type="text"
                className="border-2 w-9/12 h-10 rounded-md"
                id="comment"
                name="comment"
